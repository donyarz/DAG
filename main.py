...
def define_fixed_graph():
    """Defines a simple, fixed DAG for demonstration."""
    ...
    # Assign specific WCETs
    execution_times = {
        "N1": 10,
        "N2": 8,
        "N3": 12
    }
    return nodes, edges, execution_times
...
def generate_accesses_and_lengths_for_fixed_graph(num_tasks: int, num_resources: int = 6) -> tuple[dict, dict]:
    # This function will be adjusted for the specific graph
    ...
    # For Task T1 (the single task we are defining):
    task_idx = 0  # Corresponds to task_id 1

    # Node N1 requires R1 for 5 units
    accesses["R1"][task_idx] = 1
    lengths["R1"][task_idx].append(5)

    # Node N2 requires R2 for 3 units
    accesses["R2"][task_idx] = 1
    lengths["R2"][task_idx].append(3)
...
def allocate_resources_to_nodes(task_info: dict, task_id: int, accesses: dict, lengths: dict) -> tuple[dict, dict]:
    ...
    for node_id in nodes:
        ...
        # Check specific resource needs based on our fixed graph
        if node_id == "N1" and task_id == 1:
            if accesses["R1"][task_id - 1] > 0 and lengths["R1"][task_id - 1]:
                # Assume the first available length is for this node
                access_length = lengths["R1"][task_id - 1].pop(0)  # Use pop to ensure it's used once
                node_resource_sections.append(("R1", access_length))
        elif node_id == "N2" and task_id == 1:
            if accesses["R2"][task_id - 1] > 0 and lengths["R2"][task_id - 1]:
                access_length = lengths["R2"][task_id - 1].pop(0)
                node_resource_sections.append(("R2", access_length))
        ...
        # For simplicity, let's put normal section, then resource sections, then remaining normal
        # If a resource section is defined, it takes priority in the middle

        # Determine how to split the normal execution time around the resource sections
        # We'll aim for a simple split: some normal, then resource, then some normal

        if node_resource_sections:
            # Assuming one resource section per node for this simple example
            resource_section_type, resource_section_length = node_resource_sections[0]

            # Simple split: allocate half of remaining normal time before, half after
            # If total_resource_time_for_node > original_wcet, then normal_part_1 or 2 could be 0.
            normal_part_1 = remaining_normal_time // 2
            normal_part_2 = remaining_normal_time - normal_part_1

            if normal_part_1 > 0:
                final_allocation_for_node.append(("Normal", normal_part_1))
            final_allocation_for_node.append((resource_section_type, resource_section_length))
            if normal_part_2 > 0:
                final_allocation_for_node.append(("Normal", normal_part_2))
        else:
            # If no resource section, all time is normal
            if original_wcet > 0:
                final_allocation_for_node.append(("Normal", original_wcet))

        # Filter out zero-duration normal sections
        filtered_allocation = []
...
def generate_base_task(task_id: int, accesses_data: dict, lengths_data: dict) -> Optional[BaseTask]:
    """
    Generates a base task template with the predefined DAG, execution times, and resource allocations.
    """
    if task_id != 1:  # We are only defining one specific task
        return None

    nodes, edges, execution_times_initial = define_fixed_graph()
    ...
    # Define a fixed period for the single task
    period = 40  # A period that allows for scheduling this task multiple times
...
def generate_tasks(num_base_tasks: int, num_resources: int) -> List[BaseTask]:
    ...
    # Adjusted to generate for a single task (task_id=1)
    accesses_data, lengths_data = generate_accesses_and_lengths_for_fixed_graph(1, num_resources)

    # We only generate one task, with task_id = 1
    task = generate_base_task(task_id=1, accesses_data=accesses_data, lengths_data=lengths_data)
    if task is not None:
...
def calculate_total_processors(tasks: List[BaseTask]):
    ...
    U_norm = 0.5  # Fixing U_norm for predictable processor count
...
def run_complete_example():
    print("=== Starting Complete Example with Fixed Graph ===")

    # 1. Generate Base Tasks (Only one, using the predefined graph)
    print("\n1. Generating Base Task (Fixed Graph)...")
    num_base_tasks = 1  # We are scheduling only one type of task
    num_resources = 2  # Only R1 and R2 are needed for this example
